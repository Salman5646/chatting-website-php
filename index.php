...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  <div class="container-fluid">
    ...
    <!-- Search input -->
    <input class="form-control me-2" id="txt" type="search" placeholder="Search here" style="max-width: 50%;">
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel" style="width: 40%;height:fit-content">
  ...
  <div class="offcanvas-body">
    ...
    <a class="nav-link tab-link" href="friendrequests.php">
      Requests
      <?php
        // Show count badge if there are pending friend requests
        $con = mysqli_connect("sql209.byethost17.com", "b17_40616871", "Salman@56", "b17_40616871_main");
...
<section class="hero py-4">
  ...
  <li class="nav-item tabs tab">
    <a class="nav-link tab-link"  href="addfriends.php" data-bs-toggle="tooltip" data-bs-placement="top" title="Users">
      <i class="bi bi-people"></i>
    </a>
  </li>
  <li class="nav-item tabs tab">
    <a class="nav-link tab-link active" aria-current="page" href="index.php" data-bs-toggle="tooltip" data-bs-placement="top" title="Chats">
...
JOIN 
    friends f ON (
        (f.user1 = '$cur' AND f.user2 = a.username) OR
        (f.user2 = '$cur' AND f.user1 = a.username)
    )
LEFT JOIN 
    message m ON (
        (m.user = '$cur' AND m.from = a.username) OR 
        (m.from = '$cur' AND m.user = a.username)
    )
WHERE 
    f.status = 'accepted'
GROUP BY 
...
";
        ...
        if(mysqli_num_rows($result)>0)
        {
            while($row=mysqli_fetch_assoc($result)){
                echo '<div class="flex-hero searchable" onclick="openChat(\'' . $row["username"] . '\')">';
